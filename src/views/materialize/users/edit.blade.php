...
<?php
    ...
    if ($isProfileUpdate) {
        $profileFormAction = route('seneschal.profile.update');
        $passwordFormAction = route('seneschal.profile.password');
    } else {
        $profileFormAction =  route('seneschal.users.update', $user->hash);
        $passwordFormAction = route('seneschal.password.change', $user->hash);
    }
?>
...
<?php $customFields = config('seneschal.additional_user_fields'); ?>
...
<div class="row">
    <div class="col l6 offset-l3 m8 offset-m2 s12">
        <form method="POST" action="{{ $profileFormAction }}" accept-charset="UTF-8" role="form">
            ...
            @foreach(config('seneschal.additional_user_fields') as $field => $rules)
                <div class="row">
...
<div class="row">
    <div class="col l6 offset-l3 m8 offset-m2 s12">
        <form method="POST" action="{{ route('seneschal.users.memberships', $user->hash) }}" accept-charset="UTF-8" role="form">
            <input name="_token" value="{{ csrf_token() }}" type="hidden">
